...
export async function cacheEncryptedMessages(username, rows) {
    if (!rows.length) {
        return;
    }
...
export async function cacheDecryptedMessage(username, id, plaintext, row) {
    const db = await openDb();
    await new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_MESSAGES, "readwrite");
        const store = tx.objectStore(STORE_MESSAGES);
...
export async function loadCachedMessages(username, limit = 800, ttlMs) {
    const db = await openDb();
...
export async function pruneCachedMessages(username, maxEntries = 2000, ttlMs) {
    const db = await openDb();
    await new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_MESSAGES, "readwrite");
        const store = tx.objectStore(STORE_MESSAGES);
...
export async function clearCachedMessages(username) {
    const db = await openDb();
    await new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_MESSAGES, "readwrite");
        const store = tx.objectStore(STORE_MESSAGES);
...
export async function cacheMediaItems(username, items) {
    if (!items.length) {
        return;
    }
    const db = await openDb();
...
export async function loadCachedMedia(username, limit = 800, ttlMs) {
    const db = await openDb();
    return new Promise((resolve, reject) => {
...
export async function pruneCachedMedia(username, maxEntries = 2000, ttlMs) {
    const db = await openDb();
    await new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_MEDIA, "readwrite");
        const store = tx.objectStore(STORE_MEDIA);
...
export async function clearCachedMedia(username) {
    const db = await openDb();
    await new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_MEDIA, "readwrite");
        const store = tx.objectStore(STORE_MEDIA);
...
export async function getCacheStats(username) {
    const db = await openDb();
